...
export const useUploadDocumentFile = (
  options?: UseMutationOptions<{ filepath: string }, Error, { user_id: string; file: File; deadline?: Date }>
) => useMutation({
  mutationKey: ["upload-document-file"],
  mutationFn: async ({ user_id, file, deadline }: { user_id: string; file: File; deadline?: Date }) => {
    const formData = new FormData();
    formData.append("user_id", user_id);
    formData.append("files", file);
